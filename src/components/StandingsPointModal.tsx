...
import type { TeamPointBreakdown, TeamStanding } from "@/types";
import { formatDateShort } from "@/lib/standings";

interface StandingsPointModalProps {
...
function CompactBreakdown({ b }: { b: TeamPointBreakdown }) {
  const items = [
  ...
  return (
    <div className="rounded-lg bg-gray-50 border border-gray-100 px-3 py-2.5">
      ...
      <div className="flex justify-between items-center border-t border-gray-200 mt-2 pt-2">
        <span className="text-sm font-bold text-primary-dark">合計</span>
        <span className="text-lg font-bold text-primary tabular-nums">{formatPoints(b.total)}</span>
      </div>
    </div>
  );
}
...
export default function StandingsPointModal({
  standing,
  onClose,
  hidePoints = false,
}: StandingsPointModalProps) {
  useEffect(() => {
    if (!standing) return;
      ...
      >
        ...
        <div className="px-5 py-4 border-b border-gray-100 flex items-start justify-between gap-3 shrink-0">
          <div className="min-w-0">
            <p className="text-xs text-gray-400 mb-1">第{standing.rank}位</p>
            <h2 id="point-modal-title" className="font-bold text-lg text-primary-dark truncate">
              {standing.team.name}
            </h2>
            {standing.team.shortName !== standing.team.name && (
              <p className="text-sm text-gray-500 truncate">{standing.team.shortName}</p>
            )}
          </div>
          <button
            type="button"
        ...
        <div className="px-5 py-4 overflow-y-auto flex-1 space-y-5">
          <section>
            ...
            <SeasonStatsGrid standing={standing} hidePoints={hidePoints} />
          </section>
